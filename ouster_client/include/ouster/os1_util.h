...
template <typename iterator_type, typename F, typename C, typename Z>
std::function<void(const uint8_t*, iterator_type it)> batch_to_iter_with_packet(
    const std::vector<double>& xyz_lut, int W, int H,
    const typename iterator_type::value_type& empty, C&& c, bool& pub_raw_points, Z&& z,  F&& f) {
    int next_m_id{W};
    int32_t cur_f_id{-1};
